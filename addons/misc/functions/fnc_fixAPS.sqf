...
if (diw_armor_plates_main_disallowFriendfire &&
    {!isNull _shooter && {
...
if (_hitPoint isEqualTo "ace_hdbracket") exitWith {
    ...
    private _damageStructural = _unit getVariable ["diw_armor_plates_main_$#structural", [0,0]];
    ...
    if (_receivedDamage > 1E-3) then {
        if (diw_armor_plates_main_showDamageMarker && {(call CBA_fnc_currentUnit) isEqualTo _unit}) then {
            [_unit, [_instigator, _shooter] select (isNull _instigator), _newDamage] call diw_armor_plates_main_fnc_showDamageFeedbackMarker;
        };

        private _isTorso = _woundedHitPoint isEqualTo "Body";
        if (diw_armor_plates_main_protectOnlyTorso && {!_isTorso}) exitWith {
            // let ace do the thing
        ...
        private _damageLeft = [_unit, _receivedDamage, _actualDamage, [_instigator, _shooter] select (isNull _instigator), _ammo, _isTorso] call diw_armor_plates_main_fnc_receiveDamageACE;
