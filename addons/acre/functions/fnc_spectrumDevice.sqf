...
if ( _transmit ) then
{
	GVAR(override) = (GVAR(override) - diag_deltaTime) max 0;
}
else
{
	GVAR(override) = (GVAR(override) + (diag_deltaTime * 0.5)) min GVAR(override_max);
};

private _progress = GVAR(override) / GVAR(override_max);
